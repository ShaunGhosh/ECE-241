...
class HashTable:
    ...
    def put(self, key, data):
        ...
        if self.slots[hashvalue] is None:                       # if the slot is not filled, a new linked list is created
            self.slots[hashvalue] = OrderedList()
            self.slots[hashvalue].add(key, data)
        else:
            if self.slots[hashvalue] is not None:                      # if the slot is filled
                if self.slots[hashvalue].search(key):                   # if the slot is filled and the new key is same as the one in the node
                    self.slots[hashvalue].search(key).setData(data)
                else:                                                  # if the slot is filled but the new key is not same as the one in the node
                    self.slots[hashvalue].add(key, data)
